        ...
        if val.startswith("qt/"):
            ...
            for remote in remotes:
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    try:
                        check_output("conan download %s -verror -f json --only-recipe -cc core.cache:storage_path=%s -r %s" % (val, tmpdir, remote), shell=True)
                    except Exception:
                        continue
                    tmp_recipe_path = json.loads(check_output("conan cache path %s -verror -cc core.cache:storage_path=%s -f json" % (val, tmpdir), shell=True).decode("ascii"))["cache_path"]
